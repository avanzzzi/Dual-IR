...
pkg_dir=Dual-IR_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/Dual-IR.pkg $pkg_dir/Dual-IR-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/Dual-IR-signed.pkg --bundle-id com.GuitarML.Dual-IR --asc-provider "$TEAM_ID" --username smartguitarml@gmail.com --password "$INSTALLER_PASS"

echo "Building disk image..."
vol_name=Install_Dual-IR-$app_version
